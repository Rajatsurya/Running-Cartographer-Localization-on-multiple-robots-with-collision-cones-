...
def amcl_pose_callback(msg,robot_name):
    ...
    if robot_name == 'tb3_0':
        ...
        x_1 = msg.pose.pose.position.x  #taking the positions of no name space robot from the topic /amcl_pose
...
def collision_cone_calculator(): #this is basically the main function which will use all the other pervious functions which were described
    ...
    rospy.Subscriber('/amcl_pose', PoseWithCovarianceStamped, amcl_pose_callback, callback_args='no_ns')#add an argument called no_ns as amcl_pose_callback requires a robot name
